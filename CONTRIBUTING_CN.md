...
关于管理模式的细则，查看[审稿员文档](REVIEWERS_CN.md)。

本项目采用扁平化的组织结构。仅设有两种角色：审稿员和贡献者。
审稿员由内部作者团队轮流担任。除此之外，外部贡献者与内部作者之间没有层级上的区别。
...
**严禁使用 merge commit**。请不要在自己的 fork 中产生任何 merge commit。如果你的 PR 中包含 merge commit，审稿员将会手动将其移除。
