...
访问并登录我们的[网页](https://beta.techmc.wiki/articles)，点击任意文章顶部的 `edit` 即可！
...
本仓库中的所有文章均以 [CC BY-NC-SA 4.0](LICENSE) 分发。提交文章或修订时，你需要确认这些内容是你的原创作品，或者你已经取得了足够授权，可以按该协议提交。

你仍然保留自己贡献内容的版权；但当你发起投稿 PR 或请求审核者合并改动时，你也同意：一旦这些内容被接受，它们就可以在 CC BY-NC-SA 4.0 下被公开发布、传播和改编。只要后续使用者遵守该协议，这一授权即持续有效且不可撤销。

请不要提交任何你无权以 CC BY-NC-SA 4.0 授权的文字、图片、示意图、截图或其他素材。贡献者署名通常通过文章 Front Matter、Pull Request 记录以及仓库历史共同保留；这些记录一起构成本站实际采用的多人署名与来源追踪机制。
...
生电的技术细节很容易让新手一头雾水。如果你的某个章节包含了非常硬核的底层解析（比如较为冷门的机制，或者底层代码解读），只要在标题末尾加上 `[!ADVANCED]` 标签即可：
...
类似的，你可以将front matter中的 `isAdvanced` 设为 `true` 来将正篇文章标记为进阶内容。

### 隐藏文本（刮刮乐）

想在文章里埋个彩蛋，或者给出思考题答案但不希望直接剧透？可以用 `<hidden>` 标签。里面的文字会被模糊（萌娘百科同款），只有当读者把鼠标悬浮在上面时才会显示出来：
